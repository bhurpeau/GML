...
class DMoN3P(nn.Module):
    def __init__(self, num_X, num_Y, num_Z, L, M, N,
                 beta=3.0, lambda_X=1e-4, lambda_Y=1e-4, lambda_Z=1e-4,
                 gamma=1.0, entropy_weight=1e-3, eps=1e-9, m_chunk=None):
        super().__init__()
    ...
    def forward(self,
                ...
                edge_index_XY, edge_weight_XY,
                edge_index_YZ, edge_weight_YZ):

        device = Sx_logits.device
        Y = self.num_Y; M = self.M; eps = self.eps
        ...
        w_xy = edge_weight_XY if edge_weight_XY is not None else torch.ones_like(j_xy, dtype=S_X.dtype, device=device)
        w_yz = edge_weight_YZ if edge_weight_YZ is not None else torch.ones_like(j_yz, dtype=S_Z.dtype, device=device)

        # degrees at Y pivot
        degX_Y = scatter_add(w_xy, j_xy, dim_size=Y) + eps
        degZ_Y = scatter_add(w_yz, j_yz, dim_size=Y) + eps
        prod_deg = degX_Y * degZ_Y
        omega = 1.0 / (prod_deg + eps)
        Mnorm = prod_deg.sum() + eps
        ...
        # regs
        reg_collapse = ( self.lambda_X * self.collapse_reg(S_X)
                       + self.lambda_Y * self.collapse_reg(S_Y)
                       + self.lambda_Z * self.collapse_reg(S_Z) )
        reg_entropy  = self.entropy_weight * (
                        self.neg_entropy(S_X, eps) + self.neg_entropy(S_Y, eps) + self.neg_entropy(S_Z, eps)
                       )

        loss = -Q + reg_collapse + reg_entropy

        return {
            "loss": loss, "Q": Q.detach(),
            "Q_obs": Q_obs.detach(), "Q_exp": Q_exp.detach(),
            "aX": aX.detach(), "aY": aY.detach(), "aZ": aZ.detach(),
            "E_XY": E_XY.detach(), "E_YZ": E_YZ.detach(),
            "alpha": alpha.detach(), "gamma": gamma_soft.detach(),
        }
