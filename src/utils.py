            ...
            try:
                for addr in ast.literal_eval(row["addresses"]):
                    if "cle_interop_ban" in addr:
                        links["ban"].append(
                            {"rnb_id": rnb_id, "ban_id": addr["cle_interop_ban"]}
                        )
            ...
            try:
                for plot in ast.literal_eval(row["plots"]):
                    if "id" in plot:
                        ...
                        links["parcelle"].append(
                            {
                                "rnb_id": rnb_id,
                                "parcelle_id": plot["id"],
                                "cover_ratio": float(ratio),
                            }
                        )
            except (ValueError, SyntaxError):
                pass
            ...
            try:
                for ext_id in ast.literal_eval(row["ext_ids"]):
                    if ext_id.get("source") == "bdnb":
                        links["bdnb"].append(
                            {"rnb_id": rnb_id, "batiment_construction_id": ext_id["id"]}
                        )
            except (ValueError, SyntaxError):
                pass
    return (
        pd.DataFrame(links["ban"]).drop_duplicates(),
        pd.DataFrame(links["parcelle"]),
        pd.DataFrame(links["bdnb"]).drop_duplicates(),
    )
...
def create_golden_datasets():
    ...
    df_construction = gpd.read_file(BDNB_PATH, layer="batiment_construction")[
    ...
    df_groupe_subset = df_groupe_compile[features_to_keep].drop_duplicates(
        subset=["batiment_groupe_id"]
    )
    gdf_bat_golden = gdf_merged.merge(
    ...
    gdf_ban = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            pd.to_numeric(gdf_ban_csv.lon), pd.to_numeric(gdf_ban_csv.lat)
        ),
        crs="EPSG:4326",
    ).to_crs(TARGET_CRS)
...
def prepare_node_features(gdf_bat, gdf_par, gdf_ban):
    ...
    median_year = pd.to_numeric(
        bat_features["ffo_bat_annee_construction"], errors="coerce"
    ).median()
    median_logs = pd.to_numeric(
        bat_features["ffo_bat_nb_log"], errors="coerce"
    ).median()
    bat_features["ffo_bat_annee_construction"] = pd.to_numeric(
        bat_features["ffo_bat_annee_construction"], errors="coerce"
    ).fillna(median_year)
    ...
    bat_features["decennie_construction"] = pd.cut(
        bat_features["ffo_bat_annee_construction"],
        bins=year_bins,
        labels=year_labels,
        right=False,
    )
    bat_features["decennie_construction"] = (
        bat_features["decennie_construction"]
        .cat.add_categories("Inconnu")
        .fillna("Inconnu")
    )
    ...
    one_hot_encoded_plu = pd.get_dummies(
        gdf_par[plu_categorical_feats], prefix="plu", dtype=int
    )

    # Normalisation de la superficie
    scaled_superficie = scaler.fit_transform(gdf_par[["superficie"]])
    scaled_superficie_df = pd.DataFrame(
        scaled_superficie, columns=["superficie"], index=gdf_par.index
    )
    ...
    scaled_shape_par_df = pd.DataFrame(
        scaled_shape_par, columns=shape_feats_par.columns, index=gdf_par.index
    )

    # Concaténation et création du tenseur final pour les parcelles
    final_par_features_df = pd.concat(
        [scaled_superficie_df, one_hot_encoded_plu, scaled_shape_par_df], axis=1
    )
...
def build_graph_from_golden_datasets(
    ...
    edge_index_bp_semantic = torch.tensor(
        bp_links[["bat_idx", "par_idx"]].values.T, dtype=torch.long
    )

    # Attributs
    cover_ratio_tensor = torch.tensor(
        bp_links["cover_ratio"].values, dtype=torch.float
    ).unsqueeze(1)
    ...
    if not gdf_bat_orphans.empty and not gdf_ban_orphans.empty:
        sjoin_geo = gpd.sjoin_nearest(
            gdf_ban_orphans[["ban_id", "geometry"]],
    ...
    final_address_links = all_address_links.dropna(
        subset=["adr_idx", "bat_idx"]
    ...
    edge_attr_ab_semantic = torch.tensor(
        link_type_dummies[["semantic", "geometric"]].values, dtype=torch.float
    )
    edge_index_ab_semantic = torch.tensor(
        final_address_links[["adr_idx", "bat_idx"]].values.T, dtype=torch.long
    )
    ...
    sjoined_par = sjoined_par[
        sjoined_par["node_idx_left"] != sjoined_par["node_idx_right"]
    ]
    ...
    edge_index_pp_spatial, edge_attr_pp_spatial = torch_geometric.utils.to_undirected(
        edge_index_temp, edge_attr=edge_attr_pp_spatial
    )
    ...
    edge_index_bp_raw = torch.tensor(
        bp_links[["bat_idx", "par_idx"]].values.T, dtype=torch.long
    )
    ...
    coords_ban = torch.tensor(
        gdf_ban.geometry.apply(lambda p: (p.x, p.y)).tolist(), dtype=torch.float
    )
    ...
    data["bâtiment", "desservi_par", "adresse"].edge_index = (
        edge_index_ab_semantic.flip(0)
    )
...
def perform_hdbscan_clustering(embeddings_tensor, node_map):
    ...
    results = [
        {"id_bat": inv_node_map.get(node_idx), "community": community_id}
        for node_idx, community_id in enumerate(labels)
        if inv_node_map.get(node_idx) is not None
    ]
...
def perform_geographic_subclustering(gdf_par, building_parcel_links, communities_df):
    ...
    final_assignments = []
    semantic_clusters = communities_df[communities_df["community"] != -1][
        "community"
    ].unique()

    for semantic_id in semantic_clusters:
        buildings_in_cluster = communities_df[
            communities_df["community"] == semantic_id
        ]["id_bat"]
        parcels_for_buildings = building_parcel_links[
            building_parcel_links["rnb_id"].isin(buildings_in_cluster)
        ]
        ...
        if len(gdf_par_subset) < 2:
            assignments = pd.DataFrame(
                {"id_bat": buildings_in_cluster, "final_community": f"{semantic_id}_0"}
            )
        ...
        touching_parcels = gpd.sjoin(
            gdf_par_subset, gdf_par_subset, how="inner", predicate="intersects"
        )
        ...
        parcel_to_block_map = {
            parcel_id: f"{semantic_id}_{block_id}"
            for block_id, block in enumerate(parcel_blocks)
            for parcel_id in block
        }

        assignments = parcels_for_buildings.rename(columns={"rnb_id": "id_bat"}).copy()
        assignments["final_community"] = assignments["parcelle_id"].map(
            parcel_to_block_map
        )
        final_assignments.append(
            assignments[["id_bat", "final_community"]].drop_duplicates(
                subset=["id_bat"]
            )
        )
