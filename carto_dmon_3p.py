...
def generate_zoning_map(gdf_bat, csv_path):
    ...
    com_92 = gpd.read_file("data/BDT_3-5_GPKG_LAMB93_D092-ED2025-06-15.gpkg", layer="commune")
    ...
    gdf_zones.plot(ax=ax, column='community', cmap='tab20', 
    ...
    gdf_structure.plot(ax=ax, column='community', cmap='tab20',
                       edgecolor='white', linewidth=0.1, zorder=3)
    
    # 4. Le contour des communes
    com_92.plot(ax=ax, facecolor='none', edgecolor='black',
                linewidth=0.8, linestyle='--', alpha=0.5, zorder=4)
    ax.set_title("Urban Morphology Identification \n(Colored areas correspond to neighbourhoods detected beyond the standard urban fabric)", fontsize=14)
    ax.set_axis_off()
